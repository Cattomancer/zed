    ...
    pub(crate) fn setup_taffy_from_fibers(
        ...
        while let Some((fiber_id, entering)) = stack.pop() {
            if entering {
	                ...
	                if has_render_node {
	                    let _ = window.with_render_node_mut(fiber_id, |node, window| {
	                        ...
	                        if layout_handled && needs_layout {
	                            let mut taffy_style = node.taffy_style(rem_size, scale_factor);

                                // Root fiber needs percent(1.0) sizing to fill the available space,
                                // even if it has a render node. This ensures layout-transparent
                                // view fibers at the root still fill the window.
                                if fiber_id == root {
                                    taffy_style.size.width = taffy::style::Dimension::percent(1.0);
                                    taffy_style.size.height = taffy::style::Dimension::percent(1.0);
                                }
                ...
                if !layout_handled && needs_layout {
                    // The root fiber needs 100% sizing to fill the available space.
                    // Without this, auto-sized roots with percentage-sized children compute to 0×0.
                    let style = if fiber_id == root {
                        let mut style = taffy::Style::default();
                        style.size.width = taffy::style::Dimension::percent(1.0);
                        style.size.height = taffy::style::Dimension::percent(1.0);
                        style
                    } else {
                        taffy::Style::default()
                    };
                    let _ = update_fiber_style(window, &fiber_id, style);
                    clear_fiber_measure_func(window, &fiber_id);
                }
...
pub(crate) fn layout_bounds(
    ...
    // View fibers are layout-transparent: they derive bounds from their child.
    // This applies to both wrapper fibers and regular view fibers with view_id.
    if let Some(child_id) = window
        .fiber
        .tree
        .view_wrapper_child(fiber_id)
        .or_else(|| window.fiber.tree.view_fiber_child(fiber_id))
    {
