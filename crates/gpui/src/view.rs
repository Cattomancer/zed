...
impl RenderNode for ViewNode {
    fn taffy_style(&self, _rem_size: Pixels, _scale_factor: f32) -> TaffyStyle {
        // Views use AUTO sizing to size to their content, with flex_grow=1 to
        // expand when available space exists. This avoids issues with percent
        // sizing when parents have AUTO sizing (percent(1.0) resolves to 0).
