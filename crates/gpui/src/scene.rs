...
struct SceneSegment {
    ...
    instanced_rects: Vec<InstancedRects>,
    instanced_lines: Vec<InstancedLines>,
...
impl Scene {
    ...
    pub(crate) fn finish(&mut self, pool: &mut SceneSegmentPool) -> SceneFinishStats {
        ...
        for segment in pool.segments.values_mut() {
            ...
            segment.instanced_rects.sort_by_key(|batch| batch.order);
            segment.instanced_lines.sort_by_key(|batch| batch.order);
        }
        if self.transient.mutated_epoch == mutation_epoch {
            ...
            self.transient.instanced_rects.sort_by_key(|batch| batch.order);
            self.transient.instanced_lines.sort_by_key(|batch| batch.order);
        }
    ...
    pub(crate) fn instanced_rects_len(&self, pool: &SceneSegmentPool) -> usize {
        self.count_segments(pool, |segment| segment.instanced_rects.len())
    }

    pub(crate) fn instanced_lines_len(&self, pool: &SceneSegmentPool) -> usize {
        self.count_segments(pool, |segment| segment.instanced_lines.len())
    }
}
