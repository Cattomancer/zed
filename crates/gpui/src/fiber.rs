...
impl FiberTree {
    ...
    pub fn create_fiber_for(&mut self, descriptor: &AnyElement) -> GlobalElementId {
        ...
        let mut fiber = Fiber::new(key, child_count);
        // Set view_id for view elements so is_view_fiber() works correctly
        // and the fiber becomes layout-transparent (children participate directly
        // in parent layout context via child_ids flattening).
        if let Some(view) = descriptor.as_any_view() {
            fiber.view_id = Some(view.entity_id());
        }
    ...
    pub fn ensure_pending_view_fiber(&mut self, view_id: EntityId) -> GlobalElementId {
        ...
        // Create ViewNode render node for proper percent(1.0) sizing.
        // This ensures view fibers get correct layout behavior.
        let slot_key = Self::key_for_id(fiber_id);
        self.render_nodes
            .insert(slot_key, Box::new(crate::view::ViewNode::new(view_id)));
    ...
    pub(crate) fn is_view_fiber(&self, id: &GlobalElementId) -> bool {
        self.fibers
            .get(Self::key_for_id(*id))
            .is_some_and(|fiber| fiber.view_id.is_some())
    }
    ...
    pub(crate) fn view_fiber_child(&self, id: &GlobalElementId) -> Option<GlobalElementId> {
        if !self.is_view_fiber(id) {
            return None;
        }
    ...
    pub(crate) fn layout_parent(&self, id: &GlobalElementId) -> Option<GlobalElementId> {
        ...
        while let Some(parent_id) = current {
            // Skip layout-transparent fibers (view wrappers and view fibers)
            if !self.is_view_wrapper(&parent_id) && !self.is_view_fiber(&parent_id) {
...
impl TraversePartialTree for FiberTree {
    ...
    fn child_ids(&self, node_id: NodeId) -> Self::ChildIter<'_> {
        ...
        fn push_child(
            ...
            // View fibers are layout-transparent: their children participate in layout
            // directly within the parent's layout context. This avoids issues with
            // ViewNode's sizing when parent containers have AUTO sizing.
            if tree.is_view_wrapper(&child_id) || tree.is_view_fiber(&child_id) {
...
impl FiberRuntimeHandle<'_> {
    ...
    pub(crate) fn expand_view_fibers(&mut self, _root_fiber: GlobalElementId, cx: &mut App) {
        ...
        while let Some(fiber_id) = queue.pop() {
            ...
            // Ensure the view fiber has ViewNode for proper layout sizing.
            // This is needed because ensure_pending_view_fiber creates ViewNode, but it might
            // have been removed or the fiber might be reused from a previous frame.
            let slot_key: DefaultKey = fiber_id.into();
            let has_view_node = self
                .window
                .fiber
                .tree
                .render_nodes
                .get(slot_key)
                .is_some_and(|node| node.node_type_id() == std::any::TypeId::of::<crate::view::ViewNode>());
            ...
            // Reconcile the rendered element as a SINGLE CHILD of the view fiber.
            // This preserves the rendered element's layout properties (flex_direction, etc.)
            // instead of flattening its children directly into the view fiber.
            // Tree structure: ViewFiber(ViewNode) -> RenderedElementFiber(DivNode) -> children
            self.window
                .fiber
                .tree
                .reconcile_children(&fiber_id, std::slice::from_ref(&element), false);

            // Process the child fiber (the rendered element) with cache_fiber_payloads
            // This creates the child's render node and recurses into grandchildren.
            let child_fiber_id = self
                .window
                .fiber
                .tree
            ...
            if let Some(child_fiber_id) = child_fiber_id {
                ...
                // Map view roots from the CHILD fiber and element (not the view fiber).
                // This ensures the fiber tree structure matches the element tree.
                new_view_fibers.clear();
                self.map_view_roots_from_element(&child_fiber_id, &element, &mut new_view_fibers);
            }
    ...
    pub(crate) fn cache_fiber_payloads(
        ...
        // This creates the element's render node (e.g., EditorViewportNode)
        // which may provide conditional_slots for dynamic children.
        //
        // This is called for:
        // 1. Regular elements without view_id
        // 2. View fibers being processed with their rendered content (e.g., EditorElement from Editor::render)
        //
        // For view fibers, the element's render node (if any) replaces ViewNode. This is correct because:
        // - The element's render node provides conditional_slots for dynamic children
        // - The element's render node provides proper taffy_style for layout
        // - ViewNode was only needed when we didn't have the rendered element yet
        let slot_key = FiberTree::key_for_id(*fiber_id);
