...
use crate::{
    ...
    rems, size, transparent_black, InstancedRects, InstancedRect, LineSegmentInstance, InstancedLines
};
...
impl Window {
    ...
    pub fn focus(&mut self, handle: &FocusHandle, cx: &mut App) {
        ...
        // current effect cycle, and only for this window. During a draw cycle, we skip this
        // deferral since focus change events are already being dispatched synchronously and
        // adding a Defer effect would cause unnecessary redraws.
        if self.invalidator.not_drawing() {
    ...
    pub fn dispatch_action(&mut self, action: Box<dyn Action>, cx: &mut App) {
        ...
        cx.defer(move |cx| {
            window
                .update(cx, |_, window, cx| {
                    // Draw the window to ensure action handlers are registered in the fiber tree.
                    // This is necessary because in test mode, flush_effects may redraw dirty
                    // windows, which clears action handlers via reset_fiber_effects.
                    window.draw(cx);
    ...
    pub fn paint_batched_rects(&mut self, rects: &[RectInstance]) {
        ...
        self.next_frame.scene.insert_primitive(&mut self.segment_pool, batch, cull);
    }
    ...
    pub fn dispatch_keystroke(&mut self, keystroke: Keystroke, cx: &mut App) -> bool {
        ...
        if let Some(input) = keystroke.key_char {
            if let Some(input_handler) = self.platform_window.take_input_handler() {
                // Call with_input_handler_mut directly instead of going through dispatch_input,
                // since we're already inside an update context and dispatch_input would try to
                // re-acquire the window context via AsyncWindowContext.update().
                let fiber_id = input_handler.fiber_id();
                self.with_input_handler_mut(fiber_id, cx, |handler, window, cx| {
