...
use gpui::{
    ...
    DispatchPhase, Edges, Element, ElementInputHandler, Entity, EntityId, Focusable as _, FontId,
    ...
    ParentElement, Pixels, Position, PressureStage, ScrollDelta, ScrollHandle, ScrollWheelEvent,
    ShapedLine, SharedString, Size, Style, Styled, TextAlign, TextElement, TextRun,
...
impl RenderOnce for EditorRunIndicatorsLayer {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        let shared_layout = self.shared_layout.borrow();
        let Some(layout) = shared_layout.as_ref() else {
            return div();
        };
    ...
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        ...
        div()
            ...
            .children(embedded_elements.into_iter().map(|embedded| {
                let embedded_id =
                    (u64::from(embedded.key.0) << 32) | u64::from(embedded.key.1);
                div()
                    .id(("line_embedded", embedded_id))
                    .absolute()
    ...
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        for (ix, block) in layout.buffer.blocks.iter_mut().enumerate() {
            ...
            let top = if let Some(row) = block.row {
                ...
                px(delta_rows as f32 * line_height_value)
            } else {
                offscreen_y
            };
...
impl EditorBufferedContentNode {
    ...
    fn update(
        ...
        self.editor.update(cx, |editor_state, cx| {
            editor_state.set_style(self.style.clone(), window, cx);
        });
...
impl gpui::RenderNode for EditorCheapScrolledOverlaysNode {
    fn taffy_style(&self, rem_size: Pixels, scale_factor: f32) -> gpui::TaffyStyle {
        let mut style = Style::default();
        // Use absolute positioning so this overlay doesn't affect the layout of siblings.
        // Without this, the parent's flex layout would split height between this and
        // EditorBufferedContentNode, causing incorrect visible line calculations.
    ...
    fn prepaint_begin(&mut self, ctx: &mut gpui::PrepaintCtx) -> gpui::PrepaintFrame {
        ...
        let shared_layout = self.shared_layout.borrow();
        if let Some(layout) = shared_layout.as_ref() {
            layout
                .buffer
                .position_map
                .interaction_state
            ...
            div().relative().w_full().h_full().children(
                diff_controls.into_iter().map(|(row, y, element)| {
                    ...
                    inline_code_actions.into_iter().map(|(origin, element)| {
                        div()
                            .id(("inline_code_actions", u32::from(origin.y)))
                            .absolute()
                            .left(origin.x)
                            .top(origin.y)
                            .child(element)
                ...
                *delta = delta.coalesce(event.delta);
                ...
                editor.update(cx, |editor, cx| {
                    ...
                    if scroll_position != current_scroll_position {
                        ...
                        let shared_layout = shared_layout.borrow();
                        if let Some(layout) = shared_layout.as_ref() {
                            layout
                                .buffer
                                .position_map
...
impl gpui::RenderNode for EditorPinnedChromeNode {
    fn taffy_style(&self, rem_size: Pixels, scale_factor: f32) -> gpui::TaffyStyle {
        let mut style = Style::default();
        // Use absolute positioning so this pinned chrome layer doesn't affect the layout
        // of its sibling (EditorScrollTransformNode). Without this, the parent's flex
        // layout would split height between them.
        style.position = Position::Absolute;
        style.inset = Edges {
            top: px(0.).into(),
            right: px(0.).into(),
    ...
    fn conditional_slots(
        ...
        let viewport_layout_for_minimap = viewport_layout.clone();
        ...
        smallvec![
            ...
            gpui::ConditionalSlot::active(1, move || {
                EditorPinnedMinimapLayer {
                    shared_viewport_layout: viewport_layout_for_minimap,
                }
                .into_any_element()
            }),
            gpui::ConditionalSlot::active(2, move || {
...
impl gpui::RenderNode for EditorViewportNode {
    fn taffy_style(&self, rem_size: Pixels, scale_factor: f32) -> gpui::TaffyStyle {
        ...
        style.size.width = relative(1.).into();
        style.size.height = relative(1.).into();
        // flex_grow allows the editor to fill available vertical space in flex containers
        style.flex_grow = 1.0;
        style.to_taffy(rem_size, scale_factor)
    }

    fn layout_begin(&mut self, _ctx: &mut gpui::LayoutCtx) -> gpui::LayoutFrame {
        gpui::LayoutFrame::handled()
    }
    ...
    fn prepaint_begin(&mut self, ctx: &mut gpui::PrepaintCtx) -> gpui::PrepaintFrame {
        ...
        let line_height = self.style.text.line_height_in_pixels(rem_size);
        let visible_lines = (ctx.bounds.size.height / line_height) as f64;

        self.editor.update(ctx.cx, |editor, cx| {
            editor.set_style(self.style.clone(), ctx.window, cx);
            editor.set_visible_line_count(visible_lines, ctx.window, cx);
        });
...
impl EditorElement {
    ...
    fn layout_breakpoints(
        ...
        self.editor.update(cx, |_editor, _cx| {
            breakpoints
                .into_iter()
    ...
    fn layout_sticky_buffer_header(
        ...
        let (header, measured_size) =
            window.with_cached_measurement(header, measurement_space, default_size);
        PinnedElementLayout {
            element: Some(header),
    ...
    fn paint_minimap(&self, minimap: &MinimapLayout, window: &mut Window, cx: &mut App) {
        ...
        let thumb_bounds = minimap.thumb_layout.thumb_bounds;
        let minimap_scroll_top = minimap.minimap_scroll_top;
        let minimap_line_height = minimap.minimap_line_height;
        let max_scroll_top = minimap.max_scroll_top;
                    ...
                    move |event: &MouseDownEvent, phase, window, cx| {
                        ...
                        editor.update(cx, |editor, cx| {
                            if !thumb_bounds.contains(&event_position) {
                                ...
                                let scroll_offset = (minimap_scroll_top
                                    + ScrollPixelOffset::from(
                                        top_position / minimap_line_height,
                                    ))
                                .min(max_scroll_top);
            ...
            window.with_text_style(Some(text_style), |window| {
                window.with_content_mask(Some(ContentMask { bounds }), |window| {
                    ...
                    let (mut snapshot, is_read_only) = self.editor.update(cx, |editor, cx| {
                        editor.set_style(style.clone(), window, cx);
                    ...
                    snapshot = self.editor.update(cx, |editor, cx| {
                        ...
                        // In fiber mode, EditorViewportNode::prepaint_begin sets visible_line_count
                        // with correct bounds. The legacy prepaint may be called with stale/zero
                        // bounds. Don't overwrite a good value (> 0) with zero.
                        let new_visible_lines = (bounds.size.height / line_height) as f64;
                        let should_set = new_visible_lines > 0.0
                            || editor.visible_line_count().unwrap_or(0.0) == 0.0;
                        if should_set {
                            editor.set_visible_line_count(new_visible_lines, window, cx);
                    ...
                    let longest_line_blame_width = self
                        ...
                        .update(cx, |editor, cx| {
                            ...
                            let (_, blame_entry) = blame
                                ...
                                .flatten()?;
                            let _element = render_inline_blame_entry(blame_entry, style, cx)?;
...
fn style_generation(style: &EditorStyle, window: &Window) -> crate::StyleGeneration {
    let rem_size = window.rem_size();
    let font_size_px = f32::from(style.text.font_size.to_pixels(rem_size));
    let line_height_px = f32::from(style.text.line_height_in_pixels(rem_size));

    let mut hasher = std::collections::hash_map::DefaultHasher::new();
    window.scale_factor().to_bits().hash(&mut hasher);
    f32::from(rem_size).to_bits().hash(&mut hasher);
    font_size_px.to_bits().hash(&mut hasher);
    line_height_px.to_bits().hash(&mut hasher);
    crate::StyleGeneration(hasher.finish())
}
